    ...
    flag = spaces.then((value: any) => {
        ...
        for (let index = 0; index < spaceList.length; index++) {
            ...
            if (spaceTime == element) {
                ...
                const recognition =
                    'faceAuthentication' in spaceList[index]
                        ? spaceList[index].faceAuthentication
                        : false;
                if (recognition) {
                    if (isWebcamConnected) {
                        ...
                        // 얼굴인식 하는 부분
                        const dialogRef: MatDialogRef<FaceRecognitionDialogComponent> =
                            dialog.open(FaceRecognitionDialogComponent, {
                        ...
                        dialogService.openDialogNegative(
                            'Face recognition security is at stake in this space.\nPlease check your webcam connection'
                        );
                        return false;
                    }
                } else {
