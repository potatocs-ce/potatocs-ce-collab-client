...
import {
  ...
  POINTER_EVENT,
  TOUCH_EVENT,
} from './canvas-config/canvas-config';
...
export class CanvasService {
  ...
  setCanvasSize(
    ...
    cursorCanvas
      .getContext('2d')
      .setTransform(zoomScale, 0, 0, zoomScale, 0, 0);
  ...
  async addEventHandler() {
    this.drawingEventListener = merge(
      ...
      (pointerInfo: {
        ...
        this.startDrawing(
          this.getPoint(pointerInfo.coordinate, this.drawingEventListenCanvas)
        );
        ...
        fromEvent(this.drawingEventListenCanvas, POINTER_EVENT.POINTER_MOVE)
          .pipe(
            takeUntil(
              fromEvent(this.drawingEventListenCanvas, POINTER_EVENT.POINTER_UP)
            ),
            takeUntil(
              fromEvent(
                this.drawingEventListenCanvas,
          ...
          .subscribe({
            next: (event: PointerEvent) => {
              this.moveDrawing(
                this.getPoint(event, this.drawingEventListenCanvas)
              );
