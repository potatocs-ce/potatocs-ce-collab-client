...
export class SideNavItemComponent {
  ...
  selectedDropDownItem: WritableSignal<NavigationDropdown | null> =
    inject(NavigationService).selectedDropDownItem;
  ...
  onOpenChange(item: NavigationDropdown) {
    ...
    if (
      this.isChildrenOf(
        ...
        item as NavigationLink | NavigationDropdown
      )
    ) {
      return;
    }
  ...
  isChildrenOf(
    ...
    return parent?.children
      .filter((child: NavigationLink | NavigationDropdown) =>
        this.isDropdown(child as NavigationDropdown)
      )
      .some((child: NavigationLink | NavigationDropdown) =>
  ...
  updateSideMenu() {
    this.sideNavService.updateSideMenu().subscribe((data: any) => {
      ...
      const element = {
        ...
        faceAuthentication:
          'faceAuthentication' in space ? space.faceAuthentication : false,
      };
      ...
      this.router.navigate([
        '/' +
          this.navItems[1].children[1].children[
            this.navItems[1].children[1].children.length - 1
          ].route,
      ]);
      const sideNavLists = {
