...
export class DialogSettingSpaceComponent implements OnInit {
  ...
  getMembers() {
    try {
      ...
      if (data.length == 0) {
        ...
        this.memberInSpace.map((data) =>
          this.commonService.checkArray(data, this.adminInSpace)
        );
      }
    } catch (error) {
          ...
          this.spacesService.deleteSpace({ spaceTime }).subscribe({
            next: (data: any) => {
              ...
              this.dialogService.openDialogPositive(
                'Successfully,the space has been deleted.'
              );
            },
            error: (err: any) => {
              console.log(err);
  ...
  quitAdmin(member_id) {
    ...
    if (this.adminInSpace.length == 1) {
      this.dialogService.openDialogNegative(
        'Need at least one space administrator'
      );
  ...
  deleteMember(member_id) {
    ...
    // if (result) {
    this.dialogService
      .openDialogConfirm('Do you want to withdraw the member?')
      .subscribe((result) => {
        if (result) {
          const data = {
            id: this.spaceInfo._id,
