...
import {
  CANVAS_CONFIG,
  DRAWING_TYPE,
} from '../../../../../../services/canvas/canvas-config/canvas-config';
...
export class WbDialogComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  async registerAnswer(f: NgForm) {
    ...
    // canvas Size 추가
    this.record.drawing.canvasSize = {
      w: this.coverCanvas.width,
      h: this.coverCanvas.height,
    };
  ...
  uploadDrawing(drawingData) {
    ...
    return this.docService.uploadBlobToMultipart(
      url,
      filename,
      blob,
      'recording'
